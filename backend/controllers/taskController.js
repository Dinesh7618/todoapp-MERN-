...
const addTask = async (req, res) => {
    ...
    const newTask = new taskModel({ title, description, completed: false, userId,priority,dueDate })
    newTask.save()
        .then(() => {
...
const editTask = (req, res) => {
    console.log("dinesh-234",req.body)
    taskModel.findByIdAndUpdate(
        req.body.task._id, // ID of the document to update
        req.body.task, // Updated data
        { new: true } // Return the updated document
      ).then(() => res.status(200).json({ message: "Task edited successfully" }))
        .catch((error) => res.status(501).json({ message: error.message }))
}
